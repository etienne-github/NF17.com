...
<title>NF17.com - Modifier mes informations personnelles</title>
...
<div class="right_menu_corner"></div>
    ...
    Navigation: <a href="index.php">Accueil</a> &lt; <a href="myaccount.php">Mon compte</a> &lt; <span class="current">Modifier mes informations personnelles</span>
...
<?php fmenuGauche($vC->fConn);?>
   ...
   <div class="center_content">
   	...
   	<div class="center_title_bar">Modification de votre profil</div>
    	...
    	<div class="prod_box_big">
            ...
            <div class="center_prod_box_big">            
				...
				<?php
					$vNewMail=$_POST['newMail'];
					$vNewMail2=$_POST['newMail2'];
					$vNewMdp=$_POST['newMdp'];
					$vNewMdp2=$_POST['newMdp2'];
					$vNewPrenom=$_POST['newPrenom'];
					$vNewNom=$_POST['newNom'];
					$vCurrentMdp=$_POST['currentMdp'];
					
					/*if(!fLogin($_SESSION['login'],$vCurrentMdp,$vC->fConn))
					{
						echo"<p class=\"resFalse\">Mot de passe errone</p>";
					}
					else 
					{
						if((isset($vNewMail))&&(!empty($vNewMail))&&($vNewMail!=''))
						{
							if($vNewMail!=$vNewMail2)
							{
								echo"<p class=\"resFalse\">Les nouvelles adresses email ne sont pas identiques</p>";
							}
							else
							{
								copyUtilisateur($vNewMail,$_SESSION['login'],$vC->fConn);
								updateForeignkey($vNewMail,$_SESSION['login'],$vC->fConn);
								deleteUser($_SESSION['login'],$vC->fConn);
								$_SESSION['login']=$vNewMail;
								
							}
						}
						if((isset($vNewMdp))&&(!empty($vNewMdp))&&($vNewMdp!=''))
						{
							if($vNewMdp!=$vNewMdp2)
							{
								echo"<p class=\"resFalse\">Les nouveaux mots de passes ne sont pas identiques</p>";
							}
							else
							{
								changePassword($_SESSION['login'],$vNewMdp,$vC->fConn);
							}
							
						}
						
						if ((isset($vNewNom))&&(!empty($vNewNom))&&($vNewNom!=''))
						{
							changeNom($_SESSION['login'],$vNewNom,$vC->fConn);	
						}
						if( (isset($vNewPrenom)) && (!empty($vNewPrenom))&&($vNewPrenom!=''))
						{
							changePrenom($_SESSION['login'],$vNewPrenom,$vC->fConn);
						}
					}*/
					...
					if((isset($vNewMdp))&&(!empty($vNewMdp))&&($vNewMdp!=''))
					{
						if($vNewMdp!=$vNewMdp2)
						{
							echo"<p class=\"resFalse\">Les nouveaux mots de passe ne sont pas identiques</p>";
							$vMdpOk =false;
						}
						else
						{
					...
					if($vMailOk&&$vMdpOk&&$vSignOK)
					{
						fModifierUtilisateur($_SESSION['login'],$vNewMail,$vNewNom,$vNewPrenom,$vNewMdp,$vC->fConn);
					}*/
					
					fModifierUtilisateur($_SESSION['login'],$vCurrentMdp,$vNewMail,$vNewMail2,$vNewNom,$vNewPrenom,$vNewMdp,$vNewMdp2,$vC->fConn);
				?>
                </div>
                    ...
                    <div class="form_row">
					 <div align="center"><a href="myAccountInfo.php" class="moncompte">Poursuivre</a>
